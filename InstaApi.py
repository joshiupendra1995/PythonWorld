...
username.send_keys('dream_in_codes')
password = webdriver.find_element_by_name('password')
password.send_keys('Hindustan@2805')
...
notnow_notifications = webdriver.find_element_by_css_selector(
    'body > div.RnEpo.Yx5HN > div > div > div > div.mt3GC > button.aOOlW.HoLwm')

notnow_notifications.click()  # comment these 2 lines ou if you dont get a pop up asking to enable notification

hashtag_list = ['travelblog', 'travelblogger', 'traveler','mariatech_']

# prev_user_list = [] - if it's the first time you run it, use this line and comment the two below
prev_user_list = pd.read_csv('D:/data.csv', delimiter=',').iloc[:,
